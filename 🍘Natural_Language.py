...
def generate_custom_wave():
    ...
    if choice == "1":
        ...
        try:
            ...
            print(
                "Invalid input. Using defaults: wave=sine, freq=440, spw=100, periods=10."
            )
            wave_type = "sine"
            freq = 440.0
            spw = 100
    ...
    out_file = input(
        "Enter name for custom wave file (e.g. my_custom_signal.wav): "
    ).strip()
    if not out_file:
...
def run_color_picker(default_bg, default_pos, default_neg):
    techy_colors = {
        "Black": "#000000",  # now first
        "Vibrant Green": "#00FF00",  # default positive
    ...
    def print_palette(title):
        print(f"\n{title}")
        for i, (name, hex_) in enumerate(techy_colors.items(), 1):
            r, g, b = int(hex_[1:3], 16), int(hex_[3:5], 16), int(hex_[5:7], 16)
            swatch = f"\033[48;2;{r};{g};{b}m  \033[0m"
            print(f" {i:2d}) {name:<20} {hex_:<7} {swatch}")
        print(f" {len(techy_colors)+1}) Custom hex code")

    def pick_one(prompt):
        while True:
            ch = input(prompt).strip()
            if not (ch.isdigit() and 1 <= int(ch) <= len(techy_colors) + 1):
                print(f" ⇒ enter a number 1–{len(techy_colors)+1}.")
                continue
            idx = int(ch)
            if idx == len(techy_colors) + 1:
                custom = input(" Enter custom hex (#rrggbb): ").strip()
                if custom.startswith("#") and len(custom) == 7:
                    return custom
                print(" Invalid hex. Try again.")
            else:
                return list(techy_colors.values())[idx - 1]

    use = input("Use custom palette? (y/N): ").strip().lower() == "y"
    if not use:
        # override defaults if they were something else
        return "#000000", "#39FF14", "#39FF14"

    print_palette("Pick BACKGROUND color:")
    bg = pick_one(f" Background (1–{len(techy_colors)+1}): ")

    print_palette("Pick POSITIVE-envelope color:")
    pos = pick_one(f" Positive (1–{len(techy_colors)+1}): ")

    print_palette("Pick NEGATIVE-envelope color:")
    neg = pick_one(f" Negative (1–{len(techy_colors)+1}): ")
...
def plot_strict_sign_colored_line(
    ax, xdata, ydata, neg_color, pos_color, linewidth=2, label="Modified Wave"
):
...
class EnvelopePlot:
    def __init__(self, wav_file, ax, bg_color, pos_color, neg_color):
        ...
        (self.faint_line,) = self.ax.plot(
            self.audio_data, color=self.canvas_pos_color, alpha=0.15, lw=1
        )
        self.drawing_pos = np.zeros(self.num_points)
        self.drawing_neg = np.zeros(self.num_points)
        (self.line_pos,) = self.ax.plot(
            [], [], color=self.canvas_pos_color, lw=2, label="Positive"
        )
        (self.line_neg,) = self.ax.plot(
            [], [], color=self.canvas_neg_color, lw=2, label="Negative"
        )
        ...
        base_name = os.path.basename(wav_file)
        self.ax.text(
            0.5,                 # x = 50% across the axes
            0.96,                # y = just above the top (100%) of the axes
            base_name,
            transform=self.ax.transAxes,
            ha="center",         # horizontal alignment: center
            va="bottom",         # vertical alignment: bottom of the text at y=1.02
            fontsize=9,
            color="gray",
            alpha=0.8,
        )
    ...
    def update_drawing(self, event):
        ...
        self.line_pos.set_data(
            np.arange(self.num_points), self.drawing_pos + self.offset
        )
        self.line_neg.set_data(
            np.arange(self.num_points), self.drawing_neg + self.offset
        )
    ...
    def reapply_colors(
        ...
        neg_color,
        faint_alpha=0.15,
        final_wave_color="#00FF00",
        final_wave_alpha=0.4,
        orig_alpha=0.6,
        mod_alpha=0.8,
    ):
...
def process_single_file():
    ...
    ax.set_aspect("auto")
    # After your existing legend call:
    leg = ax.legend(loc="upper right")
    leg.get_frame().set_alpha(0.5)

    # Add a grey-boxed text entry for p/r/u just below the legend title
    ctrl_txt = "p = preview   r = reset   u = undo"
    ax.text(
        0.05,
        0.98,  # tweak y for vertical alignment under legend
        ctrl_txt,
        ...
        va="top",
        bbox=dict(
            boxstyle="round,pad=0.3", facecolor="gray", edgecolor="none", alpha=0.5
        )
    )
    ...
    original_base = os.path.splitext(os.path.basename(wf))[0]
    wav_out = os.path.join(new_folder, f"Natural_Audio_{original_base}.wav")
    ...
    ax.set_axis_off()
    fig.savefig(
        nat_svg_path, format="svg", transparent=True, bbox_inches="tight", pad_inches=0
    )
    ax.set_axis_on()
    ...
    ax.set_axis_off()
    fig.savefig(
        cmp_svg_path, format="svg", transparent=True, bbox_inches="tight", pad_inches=0
    )
    ax.set_axis_on()
